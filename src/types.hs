...
module Types where
    ...
    instance Show (Statement next) where
        show (Decl (Vec ident sz elems) next) = "\tthrust::host_vector<type>v" ++ (show ident) ++ ";"
        show (Trans fun (Vec ident sz elems) next) = "\tthrust::transform(v" ++ show ident ++ ".begin(), v" ++ show ident ++ ".end(), " ++ fun ++ ");"
    
    instance Show ElemType where
        show (CInt) = "int"    
    
    instance Show (Fun next) where
        show (Proc t name args body next) = show t ++ " " ++ name ++
                                            "(" ++ (foldr (\(t, name) acc -> 
                                                            (show t ++ " " ++ name) ++ 
                                                            case acc of
                                                               ")" -> acc
